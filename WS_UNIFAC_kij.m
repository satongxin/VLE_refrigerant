function diff=WS_UNIFAC_kij(P,T,kij)
...
component(1).group=data(i).group2;component(2).group=data(j).group2;
R=8.31433;
...
Z_WS_UNI_PURE(1)=min(root);
else
Z_WS_UNI_PURE(1)=[isreal(root(1)),isreal(root(2)),isreal(root(3))]*root;
end
%pure2
b2=b_ii(2);
a2=a_ii(2);
...
root=roots([1,C2,C1,C0]);
if isreal(root(1)) && isreal(root(2)) && isreal(root(3))
Z_WS_UNI_PURE(2)=min(root);
else
Z_WS_UNI_PURE(2)=[isreal(root(1)),isreal(root(2)),isreal(root(3))]*root;
end
%
...
g_E_pr=Z_WS_UNI-Z_WS_UNI_PURE*x-log(Z_WS_UNI./Z_WS_UNI_PURE)*x-log((Z_WS_UNI-B)./(Z_WS_UNI_PURE-[B1,B2]))*x+A/2.82843/B*log((2*Z_WS_UNI-0.82843*B)/(2*Z_WS_UNI+4.82843*B))...
    -([A1,A2]/2.82843./[B1,B2].*log((2*Z_WS_UNI_PURE-0.82843*[B1,B2])./(2*Z_WS_UNI_PURE+4.82843*[B1,B2])))*x;
